      ...
      <div className="max-w-7xl mx-auto px-8">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
          <div className="max-w-xl">
            ...
            <p className="text-gray-500 mt-6 uppercase tracking-[0.5em] text-[10px] font-bold">Un legado de innovación visual</p>
          </div>
          <div className="flex space-x-4">
            <button className="w-12 h-12 border border-[#D4AF37]/30 flex items-center justify-center text-[#D4AF37] hover:bg-[#D4AF37] hover:text-[#001a0d] transition">
          ...
          {projects.map((project, index) => (
            <div key={index} className="group">
              <div className="relative overflow-hidden deco-border" style={{ padding: 0 }}>
                <div className="absolute inset-0 bg-[#D4AF37]/20 opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none"></div>
                <img
                  src={project.image}
                  alt={project.alt}
                  className="w-full h-[450px] object-cover transition duration-1000 group-hover:scale-110 grayscale group-hover:grayscale-0"
                />
              </div>
              <div className="mt-8">
                <div className="flex justify-between items-center mb-4">
                  <h4 className="text-2xl font-bold text-[#f1e5ac] tracking-tight" style={{ fontFamily: 'Playfair Display, serif' }}>{project.title}</h4>
                  <span className="text-[10px] tracking-[0.3em] font-bold text-[#D4AF37] border-b border-[#D4AF37]/50 pb-1">{project.tag}</span>
                </div>
